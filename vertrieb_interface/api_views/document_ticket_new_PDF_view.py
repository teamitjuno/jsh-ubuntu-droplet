...
class DocumentViewTicketNew(LoginRequiredMixin, DetailView):
    ...
    def _attach_files(self, email, vertrieb_ticket, datenblatter):
        ...
        if vertrieb_ticket.datenblatter_solar_module:
            self._attach_datenblatt_from_prices(
                email,
                SolarModulePreise.objects.get(name=vertrieb_ticket.solar_module).datenblatt,
                SolarModulePreise.objects.get(name=vertrieb_ticket.solar_module).filename,
            )

        if vertrieb_ticket.datenblatter_optimizer:
            self._attach_datenblatter(email, datenblatter, "optimizer", "Huawei Optimizer")

        if vertrieb_ticket.datenblatter_speichermodule:
            if vertrieb_ticket.speicher_model == "LUNA 2000-5-S0":
                self._attach_datenblatter(email, datenblatter, "speicher_module", "Huawei S0 Speicher 5kWh")
            elif vertrieb_ticket.speicher_model == "LUNA 2000-7-S1":
                self._attach_datenblatter(email, datenblatter, "speicher_module_huawei7", "Huawei S1 Speicher 7kWh")


        if vertrieb_ticket.datenblatter_smartmeter:
            if vertrieb_ticket.smartmeter_model == "Smart Power Sensor DTSU666H":
                self._attach_datenblatter(email, datenblatter, "huawei_smartmeter_dtsu", "Smart Power Sensor")
            elif vertrieb_ticket.smartmeter_model == "EMMA-A02":
                self._attach_datenblatter(email, datenblatter, "huawei_smartmeter_emma", "EMMA-A02")

        if vertrieb_ticket.datenblatter_wechselrichter:
            if vertrieb_ticket.hersteller == "Huawei":
                self._attach_datenblatter(email, datenblatter, "wechselrichter", "Huawei Wechselrichter")

        if vertrieb_ticket.datenblatter_wallbox:
            self._attach_datenblatt_from_prices(
                email,
                WallBoxPreise.objects.get(name=vertrieb_ticket.wallboxtyp).datenblatt,
                WallBoxPreise.objects.get(name=vertrieb_ticket.wallboxtyp).filename,
            )

        if vertrieb_ticket.datenblatter_backup_box:
