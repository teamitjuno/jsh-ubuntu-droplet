...
class TicketNewEditView(LoginRequiredMixin, VertriebCheckMixin, FormMixin, View):
    ...
    def push_and_save_angebot(self, vertrieb_ticket, user_zoho_id, form, request):
        ...
        user = vertrieb_ticket.user
        user_email = user.email
        if vertrieb_ticket.rabatt > AndereKonfigurationWerte.objects.get(name="rabatt_limit_mail").value and user_email != email_address:
            subject = f"Rabattierung >10% {vertrieb_ticket.ticket_id}"
            email_content = f"""Es wurde ein Rabatt in Höhe von {vertrieb_ticket.rabatt}% gegeben in dem Nachverkauf {vertrieb_ticket.ticket_id} für {vertrieb_ticket.name}.

Investitionskosten: {vertrieb_ticket.angebotsumme + vertrieb_ticket.rabattsumme}€
Rabatt: {vertrieb_ticket.rabattsumme}€
Angebotssumme: {vertrieb_ticket.angebotsumme}€
